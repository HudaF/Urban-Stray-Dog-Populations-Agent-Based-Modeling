...
dogs-own [ age lifespan sex rabid? sterilized? vaccinated? adoptability bred? last-litter-period]
...
to setup
  ...
  create-dogs number-of-dogs [
    ...
    set bred? false
    set last-litter-period random-in-range 0 104
    set adoptability 0
...
to set-sex ;used to assign gender to both dogs and babies
...
to dogs-get-neutered
  if rabid? = false and sterilized? = false [
    set sterilized? true
    set adoptability adoptability + 0.175
    ifelse vaccinated? = true [ set color blue ] [ set color pink ] ]
end

to rabies-spread-humans
  let xval min-pxcor
  let yval min-pycor
...
to dog-reproduce
  ...
  let breed-count int(total-breed-count / 104) ;how many dogs to be bred in 2 years
  ...
  [
    ask n-of breed-count female-breeders
      [
        let mom-dog self
        set bred? true
        set last-litter-period 0
        hatch-dogs random-in-range 1 7 ; each litter will be of 1-6 dogs
        [
          set age 0 ;baby at birth has age 0 and assigned lifespan
          set lifespan random (521)
          set-sex
          if [rabid?] of mom-dog = true [set-babies-rabid] ;if mom is rabid, rabies can transfer via milk.
          set sterilized? false
          set vaccinated? false
          set bred? false
          set last-litter-period 0
